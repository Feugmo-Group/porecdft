...
def main():
    ...
    if False:
        for method, T in it.product(solvers, temperatures):
            ...
            def c1_callable(rho_arr, output_type="jax"):
                ...
                if output_type == "jax":
                    return compute_c1(rho_arr, wd, w2_hat, w3_hat, w2vec_hat,
    ...
    if True:
        ...
        for method in solvers:
            tmp = []
            output =  OUT_RES / f"phase2_2_fmt_isotherms_{method}.csv"
    ...
    for method in solvers:
        for idx, T in enumerate(temperatures):
            axes[idx].plot(data_iso[(method, T)]["p_bar"], data_iso[(method, T)]["mmol_per_g_abs"], "-", label=f"FMT-aWBII (abs) {method}", alpha=0.7, color=COLORS[method])
            axes[idx].plot(data_iso[(method, T)]["p_bar"], data_iso[(method, T)]["mmol_per_g_exc"], "--", label=f"FMT-aWBII (excess) {method}", alpha=0.7, color=COLORS[method])
    ...
    fig.suptitle("Phase 2.2 — Self-consistent FMT-aWBII isotherm benchmark")
    fig.tight_layout()
    fig.savefig(OUT_FIG / "15_phase2_2_fmt_isotherms_compare.png", dpi=150)
    plt.close(fig)

    print(f"\nFigure: {OUT_FIG}/15_phase2_2_fmt_isotherms_comparison.png")
